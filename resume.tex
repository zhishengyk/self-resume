...
\item[] \textbf{C++ 外部数据加载库}
\item \large 独立设计并实现面向日频量化研究路径的数据加载库，将数据读取，数据处理，数据构建部分进行解耦，支持多种内存布局。
\item \large 统一了 Parquet 与 HDF5 两类数据源的读取，自动完成证券代码解析、与当日证券池对齐和缺失值标记。
\item \large 采用编译期类型推导驱动分派与运行时工厂注册相结合的扩展机制，接入新数据格式或新表结构仅需注册一个工厂实现，配置解析层与既有加载路径零改动，满足开闭原则。
\end{itemize}
...
\datedsubsection{熟悉现代 C++特性，熟悉并发编程、内存模型与无锁数据结构，熟悉 STL 容器底层实现}{}
